...
var ModalPopupsDefaults = {
    ...
    shadowSize: 5,
    shadowColor: "",
    backgroundColor: "#CCCCCC", //shade color
    borderColor: "#999999",
    titleBackColor: "#D0C5BA",
    titleFontColor: "#15428B",
    popupBackColor: "#F9E7C0",
    popupFontColor: "black",
    footerBackColor: "#D0C5BA",
...
//        {
     ...
     {
        var objXml = ModalPopupsSupport.getxmlHttp_model();
...
var ModalPopupsSupport = {
    ...
    readFile: function(filename, intoElement) {
        var xmlHttp_model = getxmlHttp_model();
        var file = filename+"?r="+Math.random();
        xmlHttp_model.open("GET", file, true);
        xmlHttp_model.onreadystatechange=function()
        {
            if (xmlHttp_model.readyState==4)
            {
                intoElement.innerHTML = xmlHttp_model.responseText;
            }
        }
        xmlHttp_model.send(null);
    },
    ...
    getxmlHttp_model: function()
    {
        var xmlHttp_model;
        try
        {  // Firefox, Opera 8.0+, Safari
            xmlHttp_model=new xmlHttp_modelRequest();
        }
        catch (e)
        {  // Internet Explorer
            try
            {
                xmlHttp_model=new ActiveXObject("Msxml2.xmlHttp_model");
            }
            catch (e)
            {
                try
                {
                    xmlHttp_model=new ActiveXObject("Microsoft.xmlHttp_model");
                }
                catch (e)
                {
